...
// 古文単語テスト（20問ランダム・ゆるい判定・直近の結果・タイマー・結果送信 / iPad最適化＆全幅対応）
...
const S = {
  ...
  primaryBtn: { width: '120px', background: '#111827', color: '#fff', border: 0, borderRadius: 10, padding: '10px 12px', fontSize: 16, fontWeight: 700 },
...
export default function App() {
  ...
  // iOSズーム抑止 & 全幅メディアクエリ（全幅指定でも影響を限定）
  useEffect(() => {
...
(function devTests(){
  ...
  console.assert(list.some(e=>normalize(user1).includes(e)||e.includes(normalize(user1))), 'partial match 1');
  console.assert(list.some(e=>normalize(user2).includes(e)||e.includes(normalize(user2))), 'partial match 2');
})();
